...
## A first example
Let's look at some examples using the `penguins` data from the [`palmerpenguins`](https://cran.r-project.org/web/packages/palmerpenguins/) package. Let's say that we want to create a contingency table showing the counts of the categorical variables `species`, `sex`, and `island`. We can use `ftable` along with `dplyr`'s `select`:
...
You can easily add row and column sums:
...
`ivo_table` has lots of options for customising the look of your table. You can change the colours and fonts used, highlight columns, rows or cells, make columns bold, add row and column sums, and more. Let's look at some examples.
